...
| **v11** | v10 diffusion + независимый SHAKE-256/128 + BDZ MPHF decoder | ✓ |

Все версии используют 20 бит на предложение. v9/v10 требуют точного совпадения с общим корпусом. В v11 encoder и decoder разделены: decoder хэширует любое предложение и пропускает хэш через MPHF без membership-проверки, поэтому не сообщает, было ли предложение в encoder-словаре.

Парольное шифрование по умолчанию — **gcmwrap** (Argon2id + AES-256-GCM); доступен и устаревший режим GPG (`gpg --symmetric`). При декодировании версия парольного метода подбирается автоматически. Для стеганографического алгоритма (v9/v10/v11) при декодировании нужна та же версия, что при кодировании.
...
| `src/grammar-v11.js`, `src/v11-*.js` | независимые MPHF encoder/decoder v11 |
...
Исследование, воспроизводимые build-скрипты и метрики v11: `research/v11/`.
